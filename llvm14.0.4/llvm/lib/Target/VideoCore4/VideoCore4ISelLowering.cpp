...
SDValue
VideoCore4TargetLowering::LowerVAARG(SDValue       Op,
...
SDValue
VideoCore4TargetLowering::LowerVASTART(SDValue       Op,
				       SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  SDValue Addr = DAG.getFrameIndex(XFI->VarArgsFrameIndex,
				   MVT::i32);
  return DAG.getStore(Op.getOperand(0),
		      dl,
		      Addr,
		      Op.getOperand(1),
		      MachinePointerInfo());
  ...
  case ISD::GlobalAddress:
    {
      ...
      return DAG.getNode(VideoCore4ISD::GLOBAL,
			 dl,
			 PtrVT,
...
///
static SDValue
__LowerCallResult(SDValue                             Chain,
		  SDValue                             InFlag,
		  ...
		  SmallVectorImpl<SDValue>           &InVals)
{
  ...
  for (unsigned i = 0, e = RVLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      Chain = DAG.getCopyFromReg(Chain,
				 dl,
				 VA.getLocReg(),
				 VA.getValVT(),
                                 InFlag).getValue(1);
...
const char*
VideoCore4TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  default: return NULL;
...
EVT
VideoCore4TargetLowering::getOptimalMemOpType(const MemOp         &Op,
